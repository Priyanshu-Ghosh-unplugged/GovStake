...
const hasUsedFreeTrial = new Set();
const agentPlayCounts = new Map();
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (request.params.name === "govstake_deposit_credits") {
        const { agentId, amount } = request.params.arguments;
    ...
    if (request.params.name === "govstake_create_escrow") {
        const { buyerId, workerId, bounty, stake } = request.params.arguments;
        // Freemium
        let isFree = false;
    ...
    if (request.params.name === "govstake_play_jackpot") {
        const { agentId, betAmount } = request.params.arguments;
